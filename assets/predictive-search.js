...
class PredictiveSearch extends HTMLElement {
  ...
  getSearchResults(searchTerm) {
    fetch(
      `/search?q=${searchTerm}&type=product,article&options[unavailable_products]=hide&options[prefix]=last&view=predictive`
    )
      .then((response) => {
        if (!response.ok) {
      ...
      .then((text) => {
        ...
        // const resultsMarkup = new DOMParser().parseFromString(text, 'text/html').querySelector('#hatch-section-predictive-search').innerHTML;
